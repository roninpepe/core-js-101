...
function getFizzBuzz(num) {
  if ((num % 3 < 1) && (num % 5 < 1)) return 'FizzBuzz';
...
function getDigitalRoot(num) {
  let n = `${num}`;
  while (n.length > 1) n = `${[...n].reduce((s, v) => Number(s) + Number(v))}`;
  return Number(n);
}
...
function getCommonDirectoryPath(pathes) {
  return pathes.reduce((p, c) => p.match(/(\w*)?\//g).filter((v, i) => v === c.match(/(\w*)?\//g)[i]).join(''));
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (const v of win) {
    if ((position[v[0]][v[1]] === position[v[2]][v[3]])
    && (position[v[2]][v[3]] === position[v[4]][v[5]])
    && position[v[0]][v[1]]) {
